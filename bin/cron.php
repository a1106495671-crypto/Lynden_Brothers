...
function autoPublishApprovedArticles() {
    ...
    foreach ($articlesToPublish as $article) {
        ...
        $autoDistribution = distribution_handle_article_published($db, (int) $article['id']);
        if (($autoDistribution['created'] ?? 0) > 0) {
            log_message("已自动创建媒体分发任务: {$autoDistribution['created']} 条，后台启动: " . (int) ($autoDistribution['started'] ?? 0) . " 条");
        }
    }

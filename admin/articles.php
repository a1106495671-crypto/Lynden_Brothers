            ...
            case 'batch_update_status':
                ...
                if (!empty($article_ids) && !empty($new_status)) {
                    ...
                    foreach ($article_ids as $article_id) {
                        ...
                        if ($stmt->execute([$workflowState['status'], $workflowState['review_status'], $workflowState['published_at'], (int) $article_id])) {
                            ...
                            if ($workflowState['status'] === 'published') {
                                $autoDistribution = distribution_handle_article_published($db, (int) $article_id);
                                $distributionCreated += (int) ($autoDistribution['created'] ?? 0);
                            }
                        }
                    }
                    ...
                    if ($updatedCount > 0) {
                        ...
                        if ($distributionCreated > 0) {
                            $message .= "，已自动创建 {$distributionCreated} 条媒体分发任务";
                        }
                    } else {
                        $error = '批量更新失败';
                    }
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">GEO评分</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php foreach ($articles as $article): ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <?php
                                        $geoScoreVal = $article['geo_score'] ?? null;
                                        if ($geoScoreVal !== null) {
                                            $gs = (int) $geoScoreVal;
                                            if ($gs >= 70) {
                                                $gsBg = '#dcfce7'; $gsColor = '#15803d'; $gsLabel = '优';
                                            } elseif ($gs >= 50) {
                                                $gsBg = '#fef9c3'; $gsColor = '#854d0e'; $gsLabel = '中';
                                            } else {
                                                $gsBg = '#fee2e2'; $gsColor = '#b91c1c'; $gsLabel = '低';
                                            }
                                            $geoIssues = json_decode((string) ($article['geo_issues'] ?? '[]'), true);
                                            $tooltip = !empty($geoIssues) ? htmlspecialchars(implode('；', $geoIssues), ENT_QUOTES) : '';
                                            echo '<span title="' . $tooltip . '" style="display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:9999px;font-size:12px;font-weight:600;background:' . $gsBg . ';color:' . $gsColor . ';">';
                                            echo $gs . ' <span style="font-weight:400;font-size:11px;">(' . $gsLabel . ')</span>';
                                            echo '</span>';
                                            $rlViolations = json_decode((string) ($article['red_line_violations'] ?? '[]'), true);
                                            if (!empty($rlViolations)) {
                                                echo '<div style="font-size:10px;color:#ef4444;margin-top:2px;">红线×' . count($rlViolations) . '</div>';
                                            }
                                        } else {
                                            echo '<span style="font-size:11px;color:#9ca3af;">—</span>';
                                        }
                                        ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">

    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            $result = $stmt->execute([
                ...
                $_POST['status'],
                isset($_POST['geo_mode']) ? 'true' : 'false',
                $_POST['geo_scenario'] ?? 'B',
                trim($_POST['geo_brand_name'] ?? ''),
                trim($_POST['geo_customer_id'] ?? ''),
                $task_id
            ]);
                    ...
                    <div class="bg-white shadow rounded-lg border-l-4 border-indigo-500">
                        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="geo_mode" id="geo_mode" <?php echo !empty($task['geo_mode']) ? 'checked' : ''; ?> class="sr-only peer" onchange="document.getElementById('geo-options').style.display=this.checked?'block':'none'">
                        ...
                        <div id="geo-options" class="px-6 py-5 space-y-5" style="display:<?php echo !empty($task['geo_mode']) ? 'block' : 'none'; ?>;">
                            <!-- 品牌名称 -->
                            <div>
                                ...
                                <input type="text" name="geo_brand_name" value="<?php echo htmlspecialchars((string)($task['geo_brand_name'] ?? ''), ENT_QUOTES); ?>"
                                       placeholder="例如：湖南文韵爱阅读"
                                ...
                                <select name="geo_customer_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    ...
                                    <?php foreach ($geo_customers as $gc): ?>
                                        <option value="<?php echo htmlspecialchars($gc['id'], ENT_QUOTES); ?>"
                                            <?php echo ((string)($task['geo_customer_id'] ?? '') === (string)$gc['id']) ? 'selected' : ''; ?>>
                            ...
                            <div>
                                ...
                                <div class="mt-2 space-y-2">
                                    <?php
                                    $curScenario = (string)($task['geo_scenario'] ?? 'B');
                                    $scenarios = [
                                    ...
                                    <label class="flex items-start cursor-pointer">
                                        <input type="radio" name="geo_scenario" value="<?php echo $val; ?>" <?php echo $curScenario === $val ? 'checked' : ''; ?>
                            ...
                            <!-- 快捷入口 -->
                            <?php if (!empty($task['geo_customer_id'])): ?>
                            <div class="pt-1">
                                <a href="customers.php?id=<?php echo htmlspecialchars((string)($task['geo_customer_id'] ?? ''), ENT_QUOTES); ?>"
                                   target="_blank" class="text-xs text-indigo-600 hover:text-indigo-800">
                                    前往客户页面管理品牌事实 →
                                </a>
                            </div>
                            <?php endif; ?>

            ...
            case 'create_model':
                ...
                if (empty($name) || empty($api_key) || empty($model_id)) {
                    ...
                    try {
                        ...
                        $stmt = $db->prepare("
                            INSERT INTO ai_models (name, version, api_key, model_id, model_type, api_url, daily_limit, priority, status, created_at, updated_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                        ");
                        
                        if ($stmt->execute([$name, $version, $encrypted_api_key, $model_id, $model_type, $api_url, $daily_limit, intval($_POST['priority'] ?? 10)])) {
                ...
                if ($id <= 0 || empty($name) || empty($model_id)) {
                    ...
                    try {
                        if ($api_key === '') {
                            $stmt = $db->prepare("
                                UPDATE ai_models 
                                SET name = ?, version = ?, model_id = ?, model_type = ?, api_url = ?, daily_limit = ?, priority = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                                WHERE id = ?
                            ");
                            $result = $stmt->execute([$name, $version, $model_id, $model_type, $api_url, $daily_limit, intval($_POST['priority'] ?? 10), $status, $id]);
                        } else {
                            $encrypted_api_key = encrypt_ai_api_key($api_key);
                            $stmt = $db->prepare("
                                UPDATE ai_models 
                                SET name = ?, version = ?, api_key = ?, model_id = ?, model_type = ?, api_url = ?, daily_limit = ?, priority = ?, status = ?, updated_at = CURRENT_TIMESTAMP
                                WHERE id = ?
                            ");
                            $result = $stmt->execute([$name, $version, $encrypted_api_key, $model_id, $model_type, $api_url, $daily_limit, intval($_POST['priority'] ?? 10), $status, $id]);
                        }
...
?>
                            ...
                            <?php foreach ($models as $model): ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <button onclick="testModel(<?php echo $model['id']; ?>, '<?php echo htmlspecialchars($model['name']); ?>', '<?php echo htmlspecialchars($model['model_type']??'chat'); ?>')" class="text-green-600 hover:text-green-900">测试</button>
    ...
    <script>
        ...
        // 测试模型连接
        function testModel(id, name, modelType) {
            const btn = event.target;
            const originalText = btn.textContent;
            btn.textContent = '测试中…';
            btn.disabled = true;
            btn.className = btn.className.replace('text-green-600 hover:text-green-900', 'text-gray-400');

            const fd = new FormData();
            fd.append('id', id);
            fd.append('csrf_token', document.querySelector('input[name="csrf_token"]')?.value || '');

            fetch('ai-model-test.php', {method:'POST', body:fd})
                .then(r => r.json())
                .then(data => {
                    btn.textContent = originalText;
                    btn.disabled = false;
                    btn.className = btn.className.replace('text-gray-400', 'text-green-600 hover:text-green-900');

                    const msgEl = document.createElement('span');
                    msgEl.className = data.ok
                        ? 'ml-2 text-xs text-green-600 font-medium'
                        : 'ml-2 text-xs text-red-500 font-medium';

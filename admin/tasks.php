...
// ── 人工任务状态更新 ───────────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_human_task') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        $error = 'CSRF验证失败';
    } else {
        $taskId    = (int) ($_POST['task_id'] ?? 0);
        $newStatus = in_array($_POST['status'] ?? '', ['pending', 'in_progress', 'done'], true)
            ? $_POST['status'] : 'pending';
        if ($taskId > 0) {
            $db->prepare("UPDATE sop_dispatched_tasks SET status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?")
               ->execute([$newStatus, $taskId]);
        }
        session_write_close();
        header('Location: ' . admin_url('tasks.php?task_type=human'));
        exit;
    }
}
...
if (isset($_GET['dispatched'])) {
    $message = '已派发任务：' . htmlspecialchars($_GET['dispatched']);
}
...
$_brandName   = $_currentCustomer['name'] ?? '';
$_customerId  = (string) ($_currentCustomer['id'] ?? '');
$_contractEnd = $_currentCustomer['contract_end_at'] ?? '';

// 人工交付任务：从 sop_dispatched_tasks 表读取（SOP 节点派发后写入）
$_htStmt = $db->prepare("
    SELECT * FROM sop_dispatched_tasks
    WHERE customer_id = ?
    ORDER BY status ASC, due_date ASC, created_at DESC
");
$_htStmt->execute([$_customerId]);
$humanTasks = $_htStmt->fetchAll(PDO::FETCH_ASSOC);

// 激活 tab：URL 参数 task_type=human 进入人工任务视图，默认 AI
$_activeTab = ($_GET['task_type'] ?? 'ai') === 'human' ? 'human' : 'ai';
...
?>
        ...
        <div class="mb-6 flex items-center gap-1 rounded-xl border border-gray-200 bg-white p-1 shadow-sm w-fit">
            <a href="<?php echo htmlspecialchars(admin_url('tasks.php?task_type=ai')); ?>" class="inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold <?php echo $_activeTab === 'ai' ? 'bg-slate-900 text-white' : 'text-gray-600 hover:bg-gray-100'; ?>">
                <i data-lucide="cpu" class="h-4 w-4"></i>AI 生成任务
            </a>
            <a href="<?php echo htmlspecialchars(admin_url('tasks.php?task_type=human')); ?>" class="inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold <?php echo $_activeTab === 'human' ? 'bg-slate-900 text-white' : 'text-gray-600 hover:bg-gray-100'; ?>">
                <i data-lucide="users" class="h-4 w-4"></i>人工交付任务
            </a>
        </div>

        <?php if ($_activeTab === 'human'): ?>
        <!-- 人工交付任务看板 -->
        <div class="mb-2 rounded-lg border border-blue-200 bg-blue-50 px-5 py-3 text-sm text-blue-800">
            <strong>人工交付任务</strong>由 SOP 节点"派发"按钮写入，由客户成功团队人工完成，不进入 AI Job Queue。
        </div>
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">当期人工交付任务</h3>
                    <?php if ($_brandName): ?>
                    <p class="mt-1 text-sm text-gray-500">当前客户：<?php echo htmlspecialchars($_brandName); ?></p>
                    <?php endif; ?>
                </div>
                <a href="<?php echo htmlspecialchars(admin_url('sop-center.php')); ?>" class="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                    <i data-lucide="git-branch" class="mr-2 h-4 w-4"></i>前往 SOP 派发
                </a>
            </div>
            <div class="divide-y divide-gray-100">
                <?php if (empty($humanTasks)): ?>
                <div class="px-6 py-10 text-center text-sm text-gray-500">
                    还没有派发任务。前往 <a href="<?php echo htmlspecialchars(admin_url('sop-center.php')); ?>" class="text-blue-600 hover:underline">SOP 节点</a>，点击"派发到任务管理"后任务会显示在这里。
                </div>
                <?php endif; ?>
                <?php foreach ($humanTasks as $ht):
                    $htStatus = (string) ($ht['status'] ?? 'pending');
                    $statusBadge = match($htStatus) {
                        'in_progress' => 'bg-blue-100 text-blue-700',
                        'done'        => 'bg-emerald-100 text-emerald-700',
                        default       => 'bg-gray-100 text-gray-600',
                    };
                    $statusLabel = match($htStatus) {
                        'in_progress' => '进行中',
                        'done'        => '已完成',
                        default       => '待开始',
                    };
                    $dueDate = (string) ($ht['due_date'] ?? '');
                    $dueTs   = $dueDate ? strtotime($dueDate) : 0;
                    $isDue   = $dueTs > 0 && $dueTs < time() && $htStatus !== 'done';
                ?>
                <div class="flex flex-col gap-3 px-6 py-4 sm:flex-row sm:items-start sm:justify-between <?php echo $htStatus === 'done' ? 'opacity-60' : ''; ?>">
                    <div class="flex items-start gap-4 min-w-0">
                        <span class="mt-0.5 shrink-0 rounded-md bg-gray-100 px-2 py-1 font-mono text-xs font-bold text-gray-600"><?php echo htmlspecialchars($ht['sop_code']); ?></span>
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="font-semibold text-gray-900"><?php echo htmlspecialchars($ht['name']); ?></span>
                                <span class="rounded-full px-2.5 py-0.5 text-xs font-medium <?php echo $statusBadge; ?>"><?php echo $statusLabel; ?></span>
                            </div>
                            <?php if (!empty($ht['kpi'])): ?>
                            <p class="mt-1 text-sm text-gray-500"><?php echo htmlspecialchars($ht['kpi']); ?></p>
                            <?php endif; ?>
                            <div class="mt-1.5 flex flex-wrap items-center gap-3 text-xs text-gray-400">
                                <?php if (!empty($ht['deliverable'])): ?>
                                <span>交付物：<span class="font-medium text-gray-600"><?php echo htmlspecialchars($ht['deliverable']); ?></span></span>
                                <?php endif; ?>
                                <?php if (!empty($ht['owner'])): ?>
                                <span>负责人：<span class="font-medium text-gray-600"><?php echo htmlspecialchars($ht['owner']); ?></span></span>
                                <?php endif; ?>
                                <?php if ($dueDate): ?>
                                <span class="<?php echo $isDue ? 'font-bold text-red-600' : ''; ?>">截止：<?php echo htmlspecialchars($dueDate); ?><?php echo $isDue ? ' ⚠ 逾期' : ''; ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    <div class="shrink-0 flex items-center gap-2">
                        <!-- 状态更新快捷按钮 -->
                        <?php if ($htStatus === 'pending'): ?>
                        <form method="POST">
                            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
                            <input type="hidden" name="action" value="update_human_task">
                            <input type="hidden" name="task_id" value="<?php echo (int) $ht['id']; ?>">
                            <input type="hidden" name="status" value="in_progress">
                            <button type="submit" class="inline-flex items-center rounded-lg border border-blue-300 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100">开始</button>
                        </form>
                        <?php elseif ($htStatus === 'in_progress'): ?>
                        <form method="POST">
                            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
                            <input type="hidden" name="action" value="update_human_task">
                            <input type="hidden" name="task_id" value="<?php echo (int) $ht['id']; ?>">
                            <input type="hidden" name="status" value="done">
                            <button type="submit" class="inline-flex items-center rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100">标记完成</button>
                        </form>
                        <?php else: ?>
                        <form method="POST">
                            <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
                            ...
                            <button type="submit" class="inline-flex items-center rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-gray-50">重置</button>
                        </form>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="border-t border-gray-100 bg-gray-50 px-6 py-3 text-xs text-gray-400">
                Worker 不处理此类任务。状态更新后 SOP 节点进度不自动回写（当前版本）。
            </div>
        </div>
        <?php else: ?>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">AI 生成任务列表</h3>
                    <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">由 Worker / Job Queue 处理</span>
                </div>
            </div>

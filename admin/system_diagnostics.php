...
function getSystemInfo() {
    $info = [];
    $port = (int) env_value('HOST_PORT', 8080);
    $info['host_port'] = $port;
    ...
    // 端口占用情况
    exec("lsof -i :" . $port . " 2>/dev/null", $port_processes);
    $info['port_processes'] = $port_processes;
            ...
            <div class="space-y-2">
                <?php foreach ($system_info['php_processes'] as $process): ?>
                    <div class="font-mono text-sm p-2 bg-gray-50 rounded">
                        ...
                        // 高亮服务器进程
                        if (strpos($process, 'localhost:' . $system_info['host_port']) !== false) {
...
<div class="bg-white shadow rounded-lg mb-6">
    <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-medium text-gray-900">端口<?php echo (int) $system_info['host_port']; ?>占用情况</h3>
    </div>
    <div class="px-6 py-4">
        <?php if (empty($system_info['port_processes'])): ?>
            <p class="text-red-600">端口<?php echo (int) $system_info['host_port']; ?>未被占用 - 服务器可能未运行</p>
        <?php else: ?>
            <div class="space-y-2">
                <?php foreach ($system_info['port_processes'] as $port_info): ?>
                    <div class="font-mono text-sm p-2 bg-gray-50 rounded">

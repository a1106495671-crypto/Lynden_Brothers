...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        switch ($action) {
            ...
            case 'update_geo_settings':
                $feishu_webhook = trim($_POST['feishu_monitor_webhook'] ?? '');
                try {
                    $geoSettings = ['feishu_monitor_webhook' => $feishu_webhook];
...
?>
            ...
            <!-- GEO监测通知设置 -->
            <div class="bg-white shadow rounded-lg mt-8 border-l-4 border-indigo-500">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">GEO监测通知</h3>
                    <p class="mt-1 text-sm text-gray-500">配置飞书 Webhook，每日监测完成后自动推送 HIGH 级别竞品超越告警</p>
                </div>
                <div class="px-6 py-6">
                    <form method="POST" class="space-y-5">
                        <input type="hidden" name="action" value="update_geo_settings">
                        <input type="hidden" name="csrf_token" value="<?php echo generate_csrf_token(); ?>">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">飞书 Webhook URL</label>
                            <p class="text-xs text-gray-400 mt-0.5 mb-1">飞书群 → 群设置 → 群机器人 → 添加机器人 → 自定义机器人 → 复制 Webhook 地址</p>
                            <input type="url" name="feishu_monitor_webhook"
                                   value="<?php echo htmlspecialchars($current_settings['feishu_monitor_webhook'] ?? '', ENT_QUOTES); ?>"
                                   placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/xxxxxxxx"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <?php if (!empty($current_settings['feishu_monitor_webhook'])): ?>
                                <p class="mt-1 text-xs text-green-600">已配置 — 每日 HIGH 级别告警将自动推送</p>
                            <?php else: ?>
                                <p class="mt-1 text-xs text-gray-400">未配置 — 告警只在系统内展示，不会推送</p>
                            <?php endif; ?>
                        </div>
                        <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                            <p class="text-xs text-gray-400">推送内容：告警关键词、竞品名、品牌命中率 vs 竞品命中率、超出百分点</p>

...
if (!function_exists('db_username')) {
    function db_username(): ?string {
        ...
        return $value === false ? 'geo_user' : trim((string) $value);
    }
}
...
if (!function_exists('db_password')) {
    function db_password(): ?string {
        ...
        return $value === false ? 'geo_password' : trim((string) $value);
    }
}
...
if (!function_exists('db_create_pgsql_pdo')) {
    function db_create_pgsql_pdo(): PDO {
        ...
        $host = trim((string) (getenv('DB_HOST') ?: 'postgres'));
        $port = trim((string) (getenv('DB_PORT') ?: '5432'));
        $dbname = trim((string) (getenv('DB_NAME') ?: 'geo_system'));

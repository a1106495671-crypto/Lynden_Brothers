...
// 网站基本配置
define('SITE_NAME', env_value('SITE_NAME', 'Lynden Brothers GEO'));
define('SITE_FULL_NAME', env_value('SITE_FULL_NAME', 'Lynden Brothers GEO'));
...
define('ADMIN_BASE_PATH', '/' . trim(env_value('ADMIN_BASE_PATH', 'dl-console'), '/'));

// SQLite 源库路径，仅用于迁移和兼容维护脚本
define('DB_PATH', db_get_sqlite_path());

// 管理员配置
define('DEFAULT_BOOTSTRAP_ADMIN_PASSWORD', 'change-this-before-delivery');
define('ADMIN_USERNAME', env_value('ADMIN_BOOTSTRAP_USERNAME', 'admin'));
define('ADMIN_PASSWORD', password_hash(env_value('ADMIN_BOOTSTRAP_PASSWORD', DEFAULT_BOOTSTRAP_ADMIN_PASSWORD), PASSWORD_DEFAULT));
define('BOOTSTRAP_ADMIN_DISPLAY_NAME', env_value('ADMIN_BOOTSTRAP_DISPLAY_NAME', '系统管理员'));
define('BOOTSTRAP_ADMIN_EMAIL', env_value('ADMIN_BOOTSTRAP_EMAIL', 'admin@example.com'));
define('BOOTSTRAP_ADMIN_WEBSITE', env_value('ADMIN_BOOTSTRAP_WEBSITE', ''));
...
// 网站设置默认值
$default_settings = [
    'site_title' => SITE_FULL_NAME,
    'site_description' => SITE_DESCRIPTION,
    'site_keywords' => SITE_KEYWORDS,
    'site_logo' => '/assets/images/lynden-brothers-logo.jpg',
    'site_favicon' => '/assets/images/lynden-brothers-favicon.jpg',
    'copyright_text' => '© 2025 ' . SITE_FULL_NAME . '. All rights reserved.',
    'contact_email' => BOOTSTRAP_ADMIN_EMAIL,
    'analytics_code' => '',
    'featured_limit' => 6,
    'per_page' => ITEMS_PER_PAGE,
    'featured_description' => SITE_DESCRIPTION,
    'author_name' => BOOTSTRAP_ADMIN_DISPLAY_NAME,
    'author_bio' => '系统管理员，负责网站内容与发布流程的维护。',
    'author_avatar' => '/assets/images/avatar.jpg',
    'author_website' => BOOTSTRAP_ADMIN_WEBSITE,
    'social_github' => '',
    'social_email' => BOOTSTRAP_ADMIN_EMAIL,
    // AI系统相关设置
...
function is_default_bootstrap_admin_password(string $password): bool {
    return trim($password) === '' || hash_equals(DEFAULT_BOOTSTRAP_ADMIN_PASSWORD, trim($password));
}
...
function ai_build_chat_completions_url($apiUrl) {
    ...
    // Support explicit full endpoints, Volcengine Ark `/api/v3`, and standard OpenAI-compatible bases.
    if (preg_match('#/(chat|responses)/completions$#i', $normalized)) {
        return $normalized;
    }

    if (preg_match('#/api/v\d+$#i', $normalized) || preg_match('#/v\d+$#i', $normalized)) {

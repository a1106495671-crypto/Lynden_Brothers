...
require_once __DIR__ . '/distribution_publisher_service.php';
require_once __DIR__ . '/geo_semantic_optimizer.php';
...
class AIEngine {
    ...
    private function generateArticleContent($task, $title_info) {
        ...
        // GEO 语义优化模式
        $geoMode      = !empty($task['geo_mode']);
        $geoOptimizer = null;
        $brandFacts   = [];
        $brandName    = '';

        if ($geoMode) {
            $geoOptimizer = new GeoSemanticOptimizer($this->db);
            $customerId   = (string) ($task['geo_customer_id'] ?? '');
            $brandName    = (string) ($task['geo_brand_name'] ?? '');
            $brandFacts   = $geoOptimizer->loadBrandFacts($customerId);
        }
        ...
        // GEO 模式：将基础提示词替换为 GEO 增强提示词
        if ($geoMode && $geoOptimizer !== null) {
            $this->touchHeartbeat('geo_building_prompt', ['task_id' => (int) $task['id']]);
            $scenario = (string) ($task['geo_scenario'] ?? 'B');
            $processed_prompt = $geoOptimizer->buildGeoPrompt(
                $processed_prompt,
                (string) ($title_info['title'] ?? ''),
                (string) ($title_info['keyword'] ?? ''),
                $brandFacts,
                $brandName,
                $scenario
            );
        ...
        $geoScore = null;
        if ($geoMode && $geoOptimizer !== null) {
            $this->touchHeartbeat('geo_scoring', ['task_id' => (int) $task['id']]);
            $geoScore = $geoOptimizer->scoreContent($content, $brandName, $brandFacts);
        }
    ...
    public function callAI($task, $prompt) {
        $api_url = ai_build_chat_completions_url($task['api_url'] ?? '');
    ...
    private function saveArticle($task, $title_info, $article_data) {
        ...
        if ($status === 'published') {
            distribution_handle_article_published($this->db, (int) $article_id);
        }

        // GEO 评分写入
        if (!empty($article_data['geo_score']) && !empty($article_data['geo_optimizer'])) {
            $customerId = (string) ($task['geo_customer_id'] ?? '');
            $article_data['geo_optimizer']->saveScore((int) $article_id, $customerId, $article_data['geo_score']);
        }
